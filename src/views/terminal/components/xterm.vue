  ...
  >
    <div class="items-center" v-for="(item, index) in filesList" :key="index">
      ...
      <div style="flex: 1; margin-left: 20px">
        <a-progress
          :percent="calculatePercent(item.currentChunk, item.totalChunks)"
        />
      </div>
    </div>
  </a-modal>
...
let path = ref<string>("ws://127.0.0.1:19528");
...
const sendCharacters = (data: any) => {
  sectermTeminalCharacters(data, websocket);
};
...
const onData = (msg: any) => {
  ...
  if (sm?.fileCmd?.cmd === v1.SectermFileCmd.TRANS_ERROR)
    message.error("文件上传失败"),
      (isStopUploading = true),
      (uploadingOpen.value = false);
  if (sm?.fileCmd?.cmd === v1.SectermFileCmd.TRANS_FILE_EXISTED)
    message.error("文件上传重复"),
      (isStopUploading = true),
      (uploadingOpen.value = false);
};
...
const sendNextChunk = async () => {
  ...
  await blobToUint8Array(blob)
    .then(async (uint8Array: Uint8Array) => {
      ...
      filesList.value[filesuploadingIndex.value].currentChunk =
        currentChunk + 1;
